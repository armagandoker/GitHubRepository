...
namespace fyiReporting.RdlDesign.Netsis.Rdl.Designer.Helpers
{
    static internal class UserControlPropertiesHelper
    {
            ...
            foreach (Control ctl in mainControl.Controls)
            {
                switch (ctl.Name)
                {
                    ...
                    case "dgFilters":
                        ...
                        dg.Columns[0].HeaderText = "Filtre Formülü";
            ...
            foreach (Control ctl in mainControl.Controls)
            {
                switch (ctl.Name)
                {
                    ...
                    case "chkPBE":
                        ...
                        break;
                    case "chkGrpHeader":
                        ctl.Text = "Grup Başlıklarını Ekle";
                        ctl.Width = 145;
                        break;
                    case "chkGrpFooter":
                        ctl.Text = "Grup Footerlarını Ekle";
                        ctl.Width = 145;
                        break;
                    case "chkRepeatHeader":
                        ctl.Text = "Grup Başlıklarını Tekrarla";
                        ctl.Width = 145;
                        break;
                    case "chkRepeatFooter":
                        ctl.Text = "Grup Footerlarını Tekrarla";
                        ctl.Width = 145;
                        break;
                    case "dgGroup":
        ...
        public static void txtPName_TextChanged(object sender, EventArgs e)
        {
            ...
            foreach (Control ctl in mainControl.Controls)
            {
                switch (ctl.Name)
                {
                    case "dgParms":
                        ...
                        gv.Columns[0].HeaderText = "Parametre Adı";
        ...
        internal static void ChangeDataSetRowsControlsPropertiesOnLoad(Control mainControl)
        {
            foreach (Control ctrl in mainControl.Controls)
            {
                switch (ctrl.Name)
                {
                    ...
                    case "dgFilters":
                        DataGridView gv = (DataGridView)ctrl;
                        gv.Columns[0].HeaderText = "Filtre ";
        ...
        internal static void AddSQLTableViewFilterToTreeView(TreeView tv)
        {
            Control parentCtl = tv.Parent;
            ...
            parentCtl.Controls.Add(lb);
            
            TextBox tb = new TextBox();
            tb.Location = new System.Drawing.Point(lb.Width, 0);
            tb.Size = new System.Drawing.Size(parentCtl.Width - lb.Width, 20);
            tb.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            tb.TextChanged += tb_TextChanged;
            tb.Tag = tv;
            parentCtl.Controls.Add(tb);

            tv.Location = new System.Drawing.Point(0, 22);
            tv.Dock = DockStyle.None;
            tv.Size = new System.Drawing.Size(parentCtl.Width, parentCtl.Height - lb.Height);
            tv.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Bottom | AnchorStyles.Top;
        }

        static void tb_TextChanged(object sender, EventArgs e)
        {
            TextBox tb = (TextBox)sender;
            TreeView tv = (TreeView)tb.Tag;
            SQLSchemaNodes tc = (SQLSchemaNodes)tv.Tag;
            tc.SetFilter(tv, tb.Text);
        }

        internal static void AddNodesToTreeViewTag(TreeView tv)
        {
            SQLSchemaNodes tc = new SQLSchemaNodes();
            AddNodesToTreeViewTag(tv.Nodes.Find("Tables", false).FirstOrDefault(), tc.Tables);
            AddNodesToTreeViewTag(tv.Nodes.Find("Views", false).FirstOrDefault(), tc.Views);
            tv.Tag = tc;
        }

        private static void AddNodesToTreeViewTag(TreeNode sourceNode, TreeNode destinationNode)
        {
            foreach (TreeNode nd in sourceNode.Nodes)
            {
                destinationNode.Nodes.Add((TreeNode)nd.Clone());
            }
        }
    ...
    internal class SQLSchemaNodes
    {
        public TreeNode Tables { get; set; }
        public TreeNode Views { get; set; }

        internal SQLSchemaNodes()
        {
            this.Tables = new TreeNode("Tables");
            this.Views = new TreeNode("Views");
        }

        internal void SetFilter(TreeView tv, string filter)
        {
            TreeNode tables = tv.Nodes.Find("Tables", false).FirstOrDefault();
            TreeNode views = tv.Nodes.Find("Views", false).FirstOrDefault();

            tables.Nodes.Clear();
            views.Nodes.Clear();

            foreach (TreeNode nd in this.Tables.Nodes)
            {
                if (nd.Text.ToUpper().StartsWith(filter.ToUpper()))
                    tables.Nodes.Add(nd);
            }
            tables.Expand();

            foreach (TreeNode nd in this.Views.Nodes)
            {
                if (nd.Text.ToUpper().StartsWith(filter.ToUpper()))
                    views.Nodes.Add(nd);
            }
